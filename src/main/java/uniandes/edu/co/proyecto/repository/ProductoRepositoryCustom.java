...
public class ProductoRepositoryCustom {
    ...
    public Document obtenerProductoConCategoria(String idOrNombre) {
        List<Document> pipeline = List.of(
            ...
            new Document("$unwind", new Document("path", "$categoria").append("preserveNullAndEmptyArrays", true))
        );

        // Ejecutar la consulta en la colección "productos" y devolver el primer resultado
        List<Document> resultado = mongoTemplate.getCollection("productos").aggregate(pipeline).into(new java.util.ArrayList<>());
